...
class CSP_classifier():
    ...
    def cross_validation_unit(self, train_x, test_x, train_y, test_y, confuMatrix=False):
        nList = list(range(1, 11))
        cList = [2**(-5), 2**(-4), 2**(-3), 2**(-2), 2**(-1)]
        cList = cList + list(range(1, 21))

        accRecord = np.zeros((len(nList), len(cList)))
        ...
        for i, n_components in enumerate(nList):
            ...
            for j, c in enumerate(cList):
                ...
                # confu = confusion_matrix(test_y.flatten(), result.flatten())
                accRecord[i, j] = curRecord
        max_acc = np.max(accRecord)
        maxInd = np.where(accRecord == max_acc)
        max_n = nList[maxInd[0][0]]
        max_c = cList[maxInd[1][0]]

        self.csp_unit_fit(max_n, np.concatenate((train_x, test_x)),
                          np.concatenate((train_y, test_y)))
        all_data_csp = self.csp_unit_transform(
        ...
        self.classifier_model.fit(all_data_csp,
                                  np.concatenate((train_y, test_y)).flatten())
        print(max_acc)
        return max_acc
    ...
    def fit(self, data, label):
        ...
        acc = self.cross_validation_unit(
            train_x, test_x, train_y.flatten(), test_y.flatten())
        return acc
...
class FBCSP_classifier(CSP_classifier):
    ...
    def cross_validation_unit(self, train_x, test_x, train_y, test_y, confuMatrix=False):
        ...
        accRecord = np.zeros((len(nList), len(mList), len(cList)))
        ...
        self.fbcsp_unit_fit(max_n, np.concatenate((train_x, test_x)),
                            np.concatenate((train_y, test_y)))
        fbcsp_data = self.fbcsp_unit_transform(
            np.concatenate((train_x, test_x)))
        self.select_K = sklearn.feature_selection.SelectKBest(mutual_info_classif, k=max_m).fit(fbcsp_data,
                                                                                                np.concatenate((train_y, test_y)))
        ...
        self.classifier_model.fit(all_data_mi,
                                  np.concatenate((train_y, test_y)).flatten())
        print(max_acc)
        return max_acc
...
if __name__ == '__main__':
    ...
    task_data = matdata['task_data'][0]
    task_label = matdata['task_label'][0]
    ...
    # # task_data = matdata['task_data'][1]
    # # task_label = matdata['task_label'][1]
    ...
    # # test_data = data[5:15]
    # # test_label = label[5:15]
        pred_label = cla.predict(test_x)
